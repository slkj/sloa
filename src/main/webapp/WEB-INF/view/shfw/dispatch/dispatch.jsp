	...
	function loadDataGrid() {
		grid = $('#listGrid').datagrid({
			...
			frozenColumns : [ [ {
				...
				formatter : function(value, row, index) {
					if (row.outcome == '1') {
						return "<div  style='background-color:green;text-align:center;margin:0px;padding:0px;color:#FFFFFF;'>已维修</div>";
					} else if (row.outcome == '0') {
						return "<div  style='background-color:#309BCF;text-align:center;margin:0px;padding:0px;color:#FFFFFF;'>待维修</div>";
					}
				}
			} ] ],
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					var s = "";
// 					s += "<a href=\"javascript:void(0)\" onclick=\"javaScript:editRow('" 
// 							+ row.id 
// 							+ "');\"><span >编辑</span></a>";
	...
	function outExcel() {
		...
		window.location.href = encodeURI("../excel/expordis?" + param);
	}
			...
			<div class="top">
				...
				<form id="gridForm">
					<div class="item">
						<div id="cot"><br/>
							<div class="shang">
								车牌号：<input id="carText" style="width: 150px" /> 类型：<select id="wxtype"
							...
							<div class="xia"><br/>
								所属部门： <input id="h_department" name="department" style="width: 180px"
									data-options="prompt:'---请选择部门---'" /> 派修时间：<input id="repairTime" class="Wdate"
									type="text" onFocus="WdatePicker({isShowClear:false,readOnly:true})" /> - <input
